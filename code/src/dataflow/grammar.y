...
unsigned int unary_minus_stack_top = 0,
			 stmt_number = 0,
			 N_params = 0,
...
short current_function_table_index = -1;	// -1 for global scope
...
bool is_declared(unsigned int s_index, unsigned int f_index, unsigned int stmt_number) {
  return
...
bool is_initialized(unsigned int s_index, unsigned int f_index, unsigned int stmt_number) {
  return
	symbol_table[s_index].decl_scope[f_index] != -1 ?
...
int is_defined_function(const char *func) {
  unsigned int index = string_hash(func, FUNCTION_TABLE_SIZE);
...
program				:		decl_list			{ build_parse_tree("program", NULL, false, 1);
												  build_abstract_syntax_tree("program", 0, ast_stack_top, 0, 0);
												};
...
decl_or_def			:		ID					{ int s_index = symbol_table_entry(yylval),
												  ...
												  build_abstract_syntax_tree(yylval, IDBLOCK, 0, 0, 0);
												}
							ASSIGN				{ build_parse_tree("=", NULL, true, 0); }
							expr				{ build_parse_tree("decl_def", NULL, false, 3);
												  build_abstract_syntax_tree("=", ASSIGNBLOCK, 2, line_number + 1, stmt_number);
												}
					|		ID					{ int s_index = symbol_table_entry(yylval),
													  f_index = current_function_table_index < 0 ? FUNCTION_TABLE_SIZE : current_function_table_index;
												  ++stmt_number;
												  if (symbol_table[s_index].decl_scope[f_index] == -1)
												  	symbol_table[s_index].decl_scope[f_index] = stmt_number;
...
func_body			:		ID					{ current_function_table_index = function_table_entry(yylval);
												  build_parse_tree("ID", yylval, true, 0);
												  build_abstract_syntax_tree(yylval, FUNCBLOCK, 0, line_number + 1, ++stmt_number);
												}
							POPEN				{ build_parse_tree("(", NULL, true, 0); }
							params				{ unsigned int stmt_count = ast_stack_top - 1;
												  while (!(ast_stack[stmt_count--]->type & FUNCBLOCK));
												  build_abstract_syntax_tree("param", 0, ast_stack_top - stmt_count - 2, 0, 0);
							...
							compound_stmt		{ current_function_table_index = -1;
												  ...
												  while (!(ast_stack[stmt_count--]->type & FUNCBLOCK));
												  build_abstract_syntax_tree("body", 0, ast_stack_top - stmt_count - 2, 0, 0);
												  Ast *tmp = ast_stack[ast_stack_top - 2];
												  ast_stack[ast_stack_top - 2] = ast_stack[--ast_stack_top];
												  build_abstract_syntax_tree(tmp->token, tmp->type, 2, tmp->line_number, tmp->stmt_number);
												  free(tmp->token);
												  free(tmp);
												};
...
param				:		var_type
							ID					{ int s_index = symbol_table_entry(yylval);
												  if (symbol_table[s_index].decl_scope[current_function_table_index] == -1)
												  	symbol_table[s_index].decl_scope[current_function_table_index] = stmt_number;
												  else {
...
while_stmt			:		WHILE				{ build_parse_tree("while", NULL, true, 0);
							...
							stmt				{ build_parse_tree("while_stmt", NULL, false, 5);
												  ...
												  build_abstract_syntax_tree("true\nblock", TRUEBLOCK, ast_stack_top - stmt_count - 2, 0, 0);
												  ...
												  build_abstract_syntax_tree("while", CONTROLBLOCK, 2, stmt_line_number, stmt_stmt_number);
												};
...
if_stmt				:		IF					{ build_parse_tree("if", NULL, true, 0);
							...
							PCLOSE				{ build_parse_tree(")", NULL, true, 0); }
							stmt				{ unsigned int stmt_count = ast_stack_top - 1;
												  while (!(ast_stack[stmt_count--]->type & CONDITION));
												  build_abstract_syntax_tree("true\nblock", TRUEBLOCK, ast_stack_top - stmt_count - 2, 0, 0);
												}
							else_stmt			{ build_parse_tree("if_stmt", NULL, false, 6);
												  ...
												  build_abstract_syntax_tree("if", CONTROLBLOCK, cond_block_offset, stmt_line_number, stmt_stmt_number);
												};
...
expr				:		ID					{ build_parse_tree("ID", yylval, true, 0);
					...
					|		expr
							OR					{ build_parse_tree("\\|\\|", NULL, true, 0);
												  build_abstract_syntax_tree("||", LOGOPBLOCK, 0, 0, 0);
												}
							expr				{ build_parse_tree("expr", NULL, false, 3);
												  Ast *tmp = ast_stack[ast_stack_top - 2];
												  ast_stack[ast_stack_top - 2] = ast_stack[--ast_stack_top];
												  build_abstract_syntax_tree(tmp->token, tmp->type, 2, 0, 0);
												  free(tmp->token);
												  free(tmp);
												}
					|		expr
					...
					|		expr
							LOGEQOP				{ build_parse_tree("LOGEQOP", yytext, true, 0);
												  build_abstract_syntax_tree(yytext, LOGOPBLOCK, 0, 0, 0);
												}
							expr				{ build_parse_tree("expr", NULL, false, 3);
												  Ast *tmp = ast_stack[ast_stack_top - 2];
												  ast_stack[ast_stack_top - 2] = ast_stack[--ast_stack_top];
					...
					|		expr
							ADDOP				{ build_parse_tree("ADDOP", yytext, true, 0);
												  build_abstract_syntax_tree(yytext, ARITHOPBLOCK, 0, 0, 0);
												}
							expr				{ build_parse_tree("expr", NULL, false, 3);
												  Ast *tmp = ast_stack[ast_stack_top - 2];
												  ast_stack[ast_stack_top - 2] = ast_stack[--ast_stack_top];
					...
					|		UNILOGOP			{ build_parse_tree("UNILOGOP", yytext, true, 0);
							...
							expr %prec UNARY	{ build_parse_tree("expr", NULL, false, 2);
												  Ast *tmp = ast_stack[ast_stack_top - 2];
												  ast_stack[ast_stack_top - 2] = ast_stack[--ast_stack_top];
												  build_abstract_syntax_tree(tmp->token, tmp->type, 1, 0, 0);
												  free(tmp->token);
												  free(tmp);
												}
					|		UNIARITHOP			{ build_parse_tree("UNIARITHOP", yytext, true, 0);
												  ...
												  if (unary_minus_stack[unary_minus_stack_top - 1]) {
												  	build_abstract_syntax_tree("0", NUMBLOCK, 0, 0, 0);
												  	build_abstract_syntax_tree(yytext, ARITHOPBLOCK, 0, 0, 0);
												  }
												}
							expr %prec UNARY	{ build_parse_tree("expr", NULL, false, 2);
					...
					|		ID					{ int f_index = is_defined_function(yylval);
							...
							POPEN				{ build_parse_tree("(", NULL, true, 0); }
							args				{ unsigned int f_index = calling_function_stack[calling_function_stack_top - 3],
												  ...
												  build_abstract_syntax_tree(function_table[f_index].id, CALLBLOCK,
					...
					|		ID					{ build_parse_tree("ID", yylval, true, 0);
												  ...
												  build_abstract_syntax_tree(yylval, IDBLOCK, 0, 0, 0);
												  int s_index = symbol_table_entry(yylval),
												  	  f_index = current_function_table_index < 0 ? FUNCTION_TABLE_SIZE : current_function_table_index;
					...
					|		NUM					{ build_parse_tree("NUM", yylval, true, 0);
												  build_parse_tree("expr", NULL, false, 1);
												  build_abstract_syntax_tree(yylval, NUMBLOCK, 0, 0, 0);
												  short i = unary_minus_stack_top - 1, is_minus = 0;
												  while (i >= 0) is_minus += unary_minus_stack[i--] ? 1 : 0;
												  constant_set = insert_into_constant_set(constant_set, is_minus % 2 ? - atoi(yylval) : atoi(yylval));
												};
