 ...
 */
void gen_parse_tree(const char *token, const char *printable, short is_terminal, short number_of_children) {
  if (is_terminal) {
...
void generate_dot_file(const char *file, const char *graph_name, GRAPH_TYPE type) {
  ...
  void traverse_parse_tree(PARSE_TREE_NODE *head) {
    if (head->is_terminal) {
      ...
      while (i--) {
        if (!strcmp(head->token, printable_tokens[i])) {
          fprintf(dot_file, "\t%hd [label=\"%s | %s\", shape=\"record\", fillcolor=\"#FFEFD5\"];\n",
            head->visualization_id, head->token, head->printable);
          break;
        }
      }
      if (i < 0)
        fprintf(dot_file, "\t%hd [label=\"%s\", fillcolor=\"#FFEFD5\", shape=\"box\"];\n", head->visualization_id, head->token);
    }
    else
      fprintf(dot_file, "\t%hd [label=\"%s\", fillcolor=\"#FFEFD5\", shape=\"circle\"];\n", head->visualization_id, head->token);
    int i = 0;
    while (i < head->number_of_children) {
      traverse_parse_tree(head->children[i]);
      fprintf(dot_file, "\t%hd -> %hd;\n", head->visualization_id, head->children[i++]->visualization_id);
    }
    return;
  }
  ...
  void traverse_ast(AST_NODE *head) {
    char xlabel[14];
    sprintf(xlabel, "xlabel=\"%hd\"", head->line_number);
    fprintf(dot_file, "\t%hd [label=\"%s\", %s fillcolor=\"#%s\", shape=\"%s\"];\n",
  ...
  if (type == PARSE_TREE)
    ...
    short *visited = (short *) calloc(meet_node_visualization_id, sizeof(short));
...
void gen_ast(const char *token, AST_NODE_TYPE type, short number_of_children, short line) {
...
void gen_cfg(CFG_NODE *cfg, AST_NODE *ast) {
  ...
  void handle_control_node(AST_NODE *node) {
    if (!strcmp(node->token, "while")) {
      ...
      cfg_node_bucket[0] = last_node = cfg->out_true = (CFG_NODE *) calloc(1, sizeof(CFG_NODE));
      ...
      if (node->number_of_children > 2) {
        cfg_node_bucket[0] = last_node = cfg->out_false = (CFG_NODE *) calloc(1, sizeof(CFG_NODE));
      ...
      cfg_node_bucket[0] = last_node->out_true = (CFG_NODE *) calloc(1, sizeof(CFG_NODE));
