...
typedef struct _PARSE_TREE_NODE {
  ...
  unsigned short is_terminal, number_of_children, visualization_id;
...
typedef struct _AST_NODE {
  char *token;
  AST_NODE_TYPE type;
  unsigned short number_of_children, line_number, visualization_id;
  struct _AST_NODE **children;
} AST_NODE;

typedef struct _CFG_NODE {
  unsigned short program_point, visualization_id;
  AST_NODE *statement;
  struct _CFG_NODE *out_true, *out_false, *in1, *in2;
} CFG_NODE;

void store_symbols(const char *);
void gen_parse_tree(const char *, const char *, unsigned short, unsigned short);
void pretty_print_parse_tree(PARSE_TREE_NODE *);
void gen_ast(const char *, AST_NODE_TYPE, unsigned short, unsigned short);
void gen_cfg(CFG_NODE *, AST_NODE *);
 
unsigned short line_number, max_line_number, parse_stack_top, ast_stack_top, parse_tree_node_id, ast_node_id, meet_node_visualization_id;
SYMBOL_TABLE_NODE *symbol_table;
PARSE_TREE_NODE *parse_tree, **parse_stack;
AST_NODE *ast, **ast_stack;
